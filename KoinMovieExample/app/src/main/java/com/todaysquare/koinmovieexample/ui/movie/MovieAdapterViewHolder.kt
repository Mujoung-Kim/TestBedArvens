...
import com.todaysquare.koinmovieexample.utils.Constants.Url.Companion.MOVIE_POSTER_BASE_URL

import kotlinx.android.extensions.LayoutContainer
import kotlinx.android.synthetic.main.single_movie_item.*

class MovieAdapterViewHolder constructor(override val containerView: View, private val picasso: Picasso
) : RecyclerView.ViewHolder(containerView), LayoutContainer {
    companion object {
        fun create(parent: ViewGroup, picasso: Picasso): MovieAdapterViewHolder {
            return MovieAdapterViewHolder(
                LayoutInflater.from(parent.context).inflate(
                    R.layout.single_movie_item, parent, false), picasso)

        }
    }

    private fun setImageFavorite(isFavorite: Boolean) {
        if (isFavorite) movie_favorite_image_view.setImageResource(R.drawable.ic_star_golden_28dp)
        else movie_favorite_image_view.setImageResource(R.drawable.ic_star_border_golden_28dp)

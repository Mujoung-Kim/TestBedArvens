...
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.CITY
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.CITY_NAME
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.CURE_DEAD
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.DEATH
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.NEW_CASE
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.NEW_CCASE
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.NEW_FCASE
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.PERCENTAGE
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.POSI_NEGA
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.RECOVERED
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.SELE_LIST
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.TOP_FIVE
import com.todaysquare.coronatestapplication.utils.Constants.Param.Companion.TOTAL_CASE

//  Country
@BindingAdapter(TOP_FIVE)
fun PieChart.setTop5ValuesChart(countryResponse: CountryResponse?) {
    countryResponse?.let {
        ...
        with(yValues) {
            ...
            val remainPercent = 100 - it.city1p.toFloat() - it.city2p.toFloat() - it.city3p.toFloat()
            - it.city4p.toFloat() - it.city5p.toFloat()

            if (remainPercent.toInt() != 0)
                add(PieEntry(remainPercent, context.getString(R.string.remain_text)))
        ...
        with(pieData) {
            ...
            descriptions.text = "도시별 확진자 상위 5곳"
...
fun PieChart.setCUreOrDeadValuesChart(countryResponse: CountryResponse?) {
    countryResponse?.let {
        ...
        with(yValues) {
            add(PieEntry(it.recoveredPercentage.toFloat(), context.getString(R.string.country_cure_rate_text)))
            add(PieEntry(it.recoveredPercentage.toFloat(), context.getString(R.string.country_death_rate_text)))

            val remainPercent = 100 - it.deathPercentage.toFloat() - it.recoveredPercentage.toFloat()

            add(PieEntry(remainPercent, context.getString(R.string.etc_text)))
        ...
        with(pieData) {
            ...
            descriptions.text = "국내 완치 및 사망 상대비율"
            description = descriptions
...
fun PieChart.setPositiveOrNegativeValuesChart(countryResponse: CountryResponse?) {
    countryResponse?.let {
        ...
        with(yValues) {
            add(PieEntry(it.checkingPercentage.toFloat(), context.getString(R.string.checking_text)))
            add(PieEntry(it.casePercentage.toFloat(), context.getString(R.string.corona_positive_text)))
            add(PieEntry(it.notcasePercentage.toFloat(), context.getString(R.string.corona_negative_text)))
...
@BindingAdapter(CITY, TOTAL_CASE)
